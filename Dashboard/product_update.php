...
$get = $_GET["update"];
$select = "SELECT * FROM `e_product` WHERE `e_product`.`product_id` = $get";
...
if(isset($_POST["p_add"])) {
    ...
    if (strtolower($img_type) == "image/png" || strtolower($img_type) == "image/jpg" || strtolower($img_type) == "image/jpeg") {
        ...
        if (move_uploaded_file($_FILES["p_img"]["tmp_name"], $target)) {
            $update = "UPDATE `e_product` SET `product_name`='$name',`product_img`='$img_name' WHERE `e_product`.`product_id` = $get";
    ...
    <div class="container">
        ...
        <div class="product_form" id="product_form">
            <form method="post" enctype="multipart/form-data">
                <div class="name">
                    <input type="text" name="p_name" placeholder="Enter Your Product Name" value="<?php echo $fetch["product_name"] ?>">
                </div>
  
                <div class="name">
                    <input type="file" name="p_img" value="<?php echo $fetch["product_img"] ?>">
                </div>
                <div class="form_btn">
